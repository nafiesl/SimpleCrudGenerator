    ...
    <div class=\"float-end\">
        @if (!Request::get('action'))
            @can('create', new {$this->full_model_name})
                <a href=\"{{ route('{$this->parent_table_name}.{$this->table_name}.index', [\${$this->single_parent_model_var_name}, 'action' => 'create']) }}\" class=\"btn btn-success\">{{ __('{$this->lang_name}.create') }}</a>
            @endcan
        @endif
    </div>
...
<div class=\"row\">
                    ...
                    <tr>
                        ...
                        <td class=\"text-center\">
                            @can('update', \${$this->single_model_var_name})
                                <a href=\"{{ route('{$this->parent_table_name}.{$this->table_name}.index', [\${$this->single_parent_model_var_name}, 'action' => 'edit', 'id' => \${$this->single_model_var_name}->id] + Request::only('page', 'q')) }}\" id=\"edit-{$this->lang_name}-{{ \${$this->single_model_var_name}->id }}\">{{ __('app.edit') }}</a>
                            @endcan
                        </td>
                    </tr>
                    @endforeach
    ...
    <div class=\"col-md-4\">
        ...
        @include('{$this->parent_table_name}.{$this->table_name}.forms')
        @endif
    </div>
</div>
@endsection
";
    ...
    public function it_creates_correct_forms_view_content()
    {
        ...
        $formViewPath = resource_path("views/{$this->parent_table_name}/{$this->table_name}/forms.blade.php");
...
@can('update', \$editable{$this->model_name})
    <form method=\"POST\" action=\"{{ route('{$this->parent_table_name}.{$this->table_name}.update', [\${$this->single_parent_model_var_name}, \$editable{$this->model_name}]) }}\" accept-charset=\"UTF-8\">
        ...
        <div class=\"mb-3\">
            ...
            <a href=\"{{ route('{$this->parent_table_name}.{$this->table_name}.index', [\${$this->single_parent_model_var_name}] + Request::only('q', 'page')) }}\" class=\"btn btn-link\">{{ __('app.cancel') }}</a>
            @can('delete', \$editable{$this->model_name})
                <a href=\"{{ route('{$this->parent_table_name}.{$this->table_name}.index', [\${$this->single_parent_model_var_name}, 'action' => 'delete', 'id' => \$editable{$this->model_name}->id] + Request::only('page', 'q')) }}\" id=\"del-{$this->lang_name}-{{ \$editable{$this->model_name}->id }}\" class=\"btn btn-danger float-end\">{{ __('app.delete') }}</a>
            @endcan
        </div>
    </form>
@endcan
...
@can('delete', \$editable{$this->model_name})
    <div class=\"card\">
        ...
        <div class=\"card-footer\">
            <form method=\"POST\" action=\"{{ route('{$this->parent_table_name}.{$this->table_name}.destroy', [\${$this->single_parent_model_var_name}, \$editable{$this->model_name}]) }}\" accept-charset=\"UTF-8\" onsubmit=\"return confirm(&quot;{{ __('{$this->lang_name}.delete_confirm') }}&quot;)\" class=\"del-form float-end\" style=\"display: inline;\">
